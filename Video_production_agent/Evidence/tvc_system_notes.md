...
| 2026-03-13 | Agent-only rebuild bootstrap completed in `Video_production_agent`: added standalone UI entrypoint with agent-root launch guard (`tvc_studio_agent_ui.py`), strict tokenized MODE_NARRATE command builder, deterministic USER_CONTEXT defaults, live node/retry/API monitor, run-artifact payload persistence (`ui_launch_payload.json`), post-production workspace wiring, and desktop shortcut generator (`make_agent_ui_shortcut.py`). Added voice preset registry + runtime propagation and post-production project engine with 20 text styles + bounded Ralph loop (`max_loops=4`). | [tvc_studio_agent_ui.py](D:/AI-Apps-In-Drive/App_Station/Video_production_agent/tvc_studio_agent_ui.py) |
| 2026-03-13 | Systematic-debugging hardening pass on agent UI/post-production: fixed post-production run discovery to exclude non-output diagnostic runs by default, added base-video fallback discovery from run mp4 artifacts when manifest target is missing, fixed Windows ffmpeg ASS render path handling by rendering from ASS cwd with basename filter, and added explicit ffmpeg stderr surfacing on failure. Verified via project create -> layer edit -> render -> Ralph loop execution path. | [tvc_postproduction.py](D:/AI-Apps-In-Drive/App_Station/Video_production_agent/tvc_postproduction.py) |
| 2026-03-13 | SOTA UI architecture overhaul landed for agent repo: split UI into modular subsystem (`ui/`), introduced tokenized Aurora/Obsidian design system, command palette (`Ctrl+K`), left-rail + bento shell + right inspector, persisted `ui_state.json`, launch payload schema `ui_launch_payload.v2`, and per-run `run_ui_snapshot.json` telemetry. Upgraded post-production editor UX with draggable layer timeline, style preview matrix (20 styles), and per-layer motion controls (curve, in/out ms, opacity, blur, offsets). Added visual regression capture utility (`ui/visual_regression.py`) and deterministic helper tests. | [ui/main_window.py](D:/AI-Apps-In-Drive/App_Station/Video_production_agent/ui/main_window.py) |
| 2026-03-13 | UI crash regression fixed: `left_nav.setCurrentRow(0)` was firing navigation before `page_stack` initialization, causing Qt callback hard-crash (`0xC0000409`) in offscreen/CI smoke. Reordered shell build so stack/pages are created before first nav selection. Added deterministic UI smoke test and verified visual regression capture now completes successfully. | [ui/main_window.py](D:/AI-Apps-In-Drive/App_Station/Video_production_agent/ui/main_window.py) |
