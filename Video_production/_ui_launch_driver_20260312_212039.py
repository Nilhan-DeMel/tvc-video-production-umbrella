...
print("[UI-DRIVER] Launching via TVCLauncher.launch_production()", flush=True)
...
while True:
    ...
    if time.time() - start > 7200:
        print("[UI-DRIVER] Timeout reached, killing process", flush=True)
        launcher.process.kill()
        break
    time.sleep(1.0)

exit_code = launcher.process.exitCode()
print(f"[UI-DRIVER] Completed. exit_code={exit_code}", flush=True)
